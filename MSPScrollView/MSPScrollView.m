...
- (void)initMainView
{
    ...
    [_mainView setContentSize:CGSizeMake(SCREENWIDTH * 3, 0)];
    [_mainView setContentOffset:CGPointMake(SCREENWIDTH, 0) animated:NO];
...
- (void)initContainer
{
    ...
    _centerImageView = [[UIImageView alloc]initWithFrame:CGRectMake(SCREENWIDTH, 0, self.frame.size.width, self.frame.size.height)];
    [self setContainer:_centerImageView];
    
    _rightImageView = [[UIImageView alloc]initWithFrame:CGRectMake(2 * SCREENWIDTH, 0, self.frame.size.width, self.frame.size.height)];
    ...
    switch (_pageControlStyle) {
        case MSPScrollViewPageControlStyleCustom:
        {
            ...
            if (_pageControlAliment == MSPScrollViewPageControlAlimentCenter) {
                [_customPageControl setFrame:CGRectMake(SCREENWIDTH / 2, self.frame.size.height * 9 / 10, _customPageControl.frame.size.width, _customPageControl.frame.size.height)];
            }else if (_pageControlAliment == MSPScrollViewPageControlAlimentRight){
                [_customPageControl setFrame:CGRectMake(SCREENWIDTH - _customPageControl.frame.size.width - 20, self.frame.size.height * 9 / 10, _customPageControl.frame.size.width, _customPageControl.frame.size.height)];
            }
        ...
        case MSPScrollViewPageControlStyleClassic:
        {
            ...
            if (_pageControlAliment == MSPScrollViewPageControlAlimentCenter) {
                [pageControl setFrame:CGRectMake(SCREENWIDTH / 2 - size.width / 2, self.frame.size.height * 9 / 10, size.width, size.height)];
            }else if (_pageControlAliment == MSPScrollViewPageControlAlimentRight){
                [pageControl setFrame:CGRectMake(SCREENWIDTH - size.width - 20, self.frame.size.height * 9 / 10, size.width, size.height)];
            }
...
{
    if (_automaticScroll) {
        NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:_autoScrollTimeInterval target:self selector:@selector(updateView) userInfo:nil repeats:_infinite];
        _timer = timer;
    }
}
